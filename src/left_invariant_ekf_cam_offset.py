...
def compute_process_model_jocobian(C_vs, r_sv_v, omega, accel):
    A = np.zeros([15, 15])
    omega_v = C_vs @ omega
    accel_v = C_vs @ accel
    omega_vee = get_lifted_form(omega_v)
    accel_vee = get_lifted_form(accel_v)
    r_vee = get_lifted_form(r_sv_v)
    wr = r_vee @ omega_v
    wr_vee = get_lifted_form(wr)
...
def main():
    ...
    T_vs = np.linalg.inv(T_sv)
    C_vs, r_sv_v = pose_to_rotation_and_translation(T_vs)
    ...
    for k in range(N):
        ...
        if k % T_imu == 0:
            ...
            # propagate rotation
            C = Rotation.from_rotvec(C_vs @ omega * dt).as_matrix()
            C_iv = C_iv @ C
            C_vi = np.linalg.inv(C_iv)

            # propagate velocity and position
            pos = pos + vel * dt - C_iv @ np.cross(C_vs @ omega, r_sv_v) * dt
            vel = vel + 1.0 * accel_i * dt

            # propagate covariance
            A = compute_process_model_jocobian(C_vs, r_sv_v, omega, accel)
